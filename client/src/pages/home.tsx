...
import { ChartLine, Github, ExternalLink, Database, Eye, Check, Star, Bitcoin, Search, Timer, ArrowUp, ArrowDown, Minus, Link as LinkIcon } from "lucide-react";
...
const STORAGE_KEY = "binance_watchlist";
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.error("Erro ao buscar exchangeInfo", e);
      } finally {
...
const NumberCell: React.FC<{ value?: string; fallback?: string; align?: "left" | "right" }>
  ...
  const { decimals } = useApp();
  const num = value ? Number(value) : NaN;
  const cls = isNaN(num) ? "" : num > 0 ? "text-green-600" : num < 0 ? "text-red-600" : "";
  const formatted = isNaN(num) ? fallback : num.toFixed(decimals);
  return (
...
const SymbolPicker: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return allSymbols.slice(0, 200);
...
const WatchTable: React.FC = () => {
  const { watchlist, removeFromWatchlist, sortBy, setSortBy } = useApp();
  const { tickers } = useBinanceTicker(watchlist);

  const rows = useMemo(() => {
    let data = watchlist.map((s) => ({ symbol: s, data: tickers[s] }));
    if (sortBy === "symbol") data = data.sort((a, b) => a.symbol.localeCompare(b.symbol));
    if (sortBy === "price") data = data.sort((a, b) => Number(b.data?.c || 0) - Number(a.data?.c || 0));
    if (sortBy === "change") data = data.sort((a, b) => Number(b.data?.P || 0) - Number(a.data?.P || 0));
    return data;
